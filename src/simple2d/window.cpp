  ...
  while (!window->close) {
      ...
      switch (e.type) {
        ...
        case SDL_KEYDOWN:
          if (window->on_key && e.key.repeat == 0) {
            S2D_Event event = {};
            memset(&event, 0, sizeof(S2D_Event));
            event.type = S2D_KEY_DOWN;
            event.key = SDL_GetScancodeName(e.key.keysym.scancode);
            window->on_key(event);
          }
        break;
        ...
        case SDL_MOUSEWHEEL:
          if (window->on_mouse) {
            S2D_Event event = { };
            memset(&event, 0, sizeof(S2D_Event));
            event.type = S2D_MOUSE_SCROLL;
            event.direction = e.wheel.direction;
            event.delta_x = e.wheel.x;
            event.delta_y = e.wheel.y;
            window->on_mouse(event);
          }
              break;
        ...
        case SDL_MOUSEMOTION:
          if (window->on_mouse) {
            ...
            S2D_Event event = { };
            event.type = S2D_MOUSE_MOVE;
            event.x = mx;
            event.y = my;
            event.delta_x = e.motion.xrel;
            event.delta_y = e.motion.yrel;
            window->on_mouse(event);
          }
              break;
        ...
        case SDL_CONTROLLERAXISMOTION:
          if (window->on_controller) {
            S2D_Event event = { };
            event.which = e.caxis.which;
            event.type = S2D_AXIS;
            event.axis = e.caxis.axis;
        ...
        case SDL_JOYAXISMOTION:
          if (window->on_controller && !S2D_IsController(e.jbutton.which)) {
            S2D_Event event = { };
            event.which = e.jaxis.which;
            event.type = S2D_AXIS;
            event.axis = e.jaxis.axis;
        ...
        case SDL_JOYDEVICEREMOVED:
          if (S2D_IsController(e.jdevice.which)) {
            S2D_Log(S2D_INFO, "Controller #%i: %s removed (%i remaining)", e.jdevice.which,
                    SDL_GameControllerName(SDL_GameControllerFromInstanceID(e.jdevice.which)), SDL_NumJoysticks());
            SDL_GameControllerClose(SDL_GameControllerFromInstanceID(e.jdevice.which));
          } else {
            S2D_Log(S2D_INFO, "Controller #%i: %s removed (%i remaining)", e.jdevice.which,
                    SDL_JoystickName(SDL_JoystickFromInstanceID(e.jdevice.which)), SDL_NumJoysticks());
    ...
    for (int i = 0; i < num_keys; i++) {
      if (window->on_key) {
        if (key_state[i] == 1) {
          S2D_Event event = { };
          event.type = S2D_KEY_HELD;
          event.key = SDL_GetScancodeName((SDL_Scancode) i);
          window->on_key(event);
        }
      }
    }
...
void S2D_Screenshot(S2D_Window *window, const char *path) {
  ...
  for (int index = 0; index < height_div_2; index++) {
    ...
    memcpy((Uint8 *) (surface->pixels) + surface->pitch * index,
           (Uint8 *) (surface->pixels) + surface->pitch * (surface->h - index - 1), surface->pitch);
